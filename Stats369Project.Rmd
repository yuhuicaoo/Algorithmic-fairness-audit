...
# Reprex Critique
We can see that example A does not use reprex to reproduce its bug reports or includes every command (libraries used, what my_url is) used, compared to examples B and C which do use reprex and includes every command used. Examples A and B both includes the entire poem to demonstrate the problem compared to example C which uses a shortened sentence (minimal data) to demonstrate the problem. Therefore, example C would be the best example of reporting a bug as it follows reprex best practices the most out of the three examples, increasing its reproducibility for others. Example C uses reprex, includes every command that is needed, uses minimal data to demonstrate the problem, example B uses reprex and includes every command / code needed but does not use minimal data to demonstrate the problem. Example A does not use reprex (but a screenshot of the bug) , does not include every command/code and uses the entire data (poem) thus has the lowest reproducibility ("Reprex do's and don'ts," n.d.).
...
2.  I am proud of this assignment as I have gained a lot of experience with R and data science practice. Although unsure of my overall performance for this assignment , I believe the experience and learning is what matters the most.

3.  One thing i would like to do differently in the next assignment is reach out for help earlier, delayed asking for clarification and help on task2 until the last opportunity.
...
2.  Reprex do's and don'ts. (n.d.). Reprex.tidyverse.org. Retrieved August 2, 2023, from <https://reprex.tidyverse.org/articles/reprex-dos-and-donts.html>
